...
contract Ownable {
  ...
  modifier onlyOwner() {
    require(
      isCallerOwner(),
      "[caller from isCallerOwner] must be the contract owner"
    );
    _;
  }
  ...
  function transferOwnership(address newOwner) public onlyOwner {
    ...
    require(newOwner != address(0), "[newOner] must be a valid address");
...
contract Pausable is Ownable {
  ...
  function contractPause() public onlyOwner whenNotPaused {
    _paused = true;
    emit Paused(msg.sender);
  }

  function contractNotPause() public onlyOwner paused {
    _paused = false;
  ...
  modifier whenNotPaused() {
    require(!_paused, "[Paused] - contract is currently paused");
    _;
  }

  modifier paused() {
    require(_paused, "[Not Paused] - contract is currently not paused");
    _;
  }
...
contract ERC721 is Pausable, ERC165 {
  ...
  function balanceOf(address owner) public view returns (uint256) {
    ...
    require(owner != address(0), "[address owner] must provide valid address");
  ...
  function ownerOf(uint256 tokenId) public view returns (address) {
    ...
    require(owner != address(0), "[address owner] muss provide valid address");
  ...
  function approve(address to, uint256 tokenId) public {
    ...
    require(owner != to, "[owner to] must address different than own address");
    // require the msg sender to be the owner of the contract or isApprovedForAll() to be true
    require(
      msg.sender == owner || isApprovedForAll(owner, msg.sender),
      "[msg.sender || isApprovedForAll] either needs to be owner of contract or enabled"
    );
  ...
  function getApproved(uint256 tokenId) public view returns (address) {
    // verifify if the tokenId exists
  ...
  function _transferFrom(
    ...
    require(
      ...
      "[ownerOf tokenId] needs to be owner of token"
    );
    // require token is being transfered to valid address
    require(to != address(0), "[address] needs to be valid");
    // clear approval
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
  ...
  function _setInternalTokenURI(uint256 tokenId) internal {
    require(_exists(tokenId), "[token] does not exist");
    _tokenURIs[tokenId] = _tokenURIs[tokenId] = strConcat(
      _baseTokenURI,
      uint2str(tokenId)
    );
  }
...
contract RLMEstate721Token is ERC721Metadata {
  ...
  function mint(address to, uint256 tokenId) public onlyOwner returns (bool) {
    super._mint(to, tokenId);
    super._setInternalTokenURI(tokenId);
    return true;
  }
}
